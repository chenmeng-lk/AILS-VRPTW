...
def run_single_test(instance, seed, max_runtime, best_cost=None, enable_tsla=False, disable_tsla=False):
    """运行单个测试实例，根据参数决定是否添加 --enable_tsla 或 --disable_tsla"""
    cmd = [
        "uv", "run", "pyvrp",
        instance,
        "--seed", str(seed),
    ...
    if enable_tsla:
        cmd.append("--enable_tsla")
    elif disable_tsla:
        cmd.append("--disable_tsla")
...
def run_multi_seed_tests_parallel(vrp_info_list, seeds, max_runtime, detailed_csv, summary_csv, workers=80, enable_tsla=False, disable_tsla=False):
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        future_to_task = {
            executor.submit(run_task, name, path, seed, runtime, cost, enable_tsla, disable_tsla): (name, seed)
            for name, path, seed, runtime, cost, enable_tsla, disable_tsla in tasks
        }
...
def main():
    ...
    parser.add_argument('--enable_tsla', action='store_true', help='启用 TSLA')
    parser.add_argument('--disable_tsla', action='store_true', help='禁用 TSLA')
    ...
    run_multi_seed_tests_parallel(
        ...
        enable_tsla=args.enable_tsla,
        disable_tsla=args.disable_tsla
    )
